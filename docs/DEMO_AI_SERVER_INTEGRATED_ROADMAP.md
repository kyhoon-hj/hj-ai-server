# 데모 개발 및 AI Server 검증·개선 통합 로드맵

작성일: 2026-08-05
상태: 실행 기준 문서
적용 범위: `demo/`, `src/`, `prisma/`, 배포 및 운영 설정

## 1. 목적

데모를 단순 시연 화면이 아니라 AI Server의 기능·계약·보안·RAG 품질·성능을 반복 검증하는 제품화 도구로 발전시킵니다. 모든 AI Server 개선은 대응하는 데모 시나리오와 자동 검증 증거를 가져야 하며, 최종적으로 외부 앱이 사용할 `/v1` 공통 API 명세와 운영 승인 기준을 확정합니다.

## 2. 진행 원칙

1. 문제를 재현하는 데모 시나리오를 먼저 정의합니다.
2. AI Server를 수정한 뒤 동일 시나리오가 PASS하는지 확인합니다.
3. 회귀 가능성이 있는 문제는 자동 계약·통합 시험으로 남깁니다.
4. DB를 직접 확인하지 않고 외부 계약은 HTTP 응답으로 판정합니다.
5. tenant 격리, 인증과 데이터 노출 문제는 성능보다 먼저 해결합니다.
6. Bedrock 호출은 항상 명시적 `maxTokens`를 사용합니다.
7. 부하는 Smoke → Baseline → Burst 순으로 높입니다.
8. 실제 질문·응답이 저장되는 리포트에는 개인정보를 사용하지 않습니다.
9. 단계별 Exit Gate를 통과하기 전 다음 단계의 운영 승인을 하지 않습니다.
...
| 단위 테스트 | 24/24 PASS | 핵심 서비스 branch 80% 이상 |
| E2E | 루트 GET 1건 | 외부·관리·지식 생명주기 전체 |
| line coverage | 30.34% | 핵심 서비스 80% 이상 |
| lint | 오류 11건 | 오류 0건 |
| dependency audit | High 5, Moderate 6 | High 0 또는 승인 예외 |
| DB migration | 최신 1건 미적용 | 배포 환경과 schema 일치 |
| 데모 | 28개 operation 수동 호출, 기본 계약/성능 러너 | 전체 자동 검증 및 승인 리포트 |
| 외부 API | 기존 내부 endpoint 혼재 | `/v1` 안정 계약 확정 |
| 관리자 API | AppInfo 무인증 | 관리자 인증·RBAC |
...
- [ ] `API-SRV-01` 외부 `/v1`, 지식 `/admin/v1/knowledge`, 플랫폼 `/admin/v1/apps` 분리
...
|---:|---|---|---|
| 1 | 기준 환경·migration | 없음 | S |
| 2 | 관리자 인증·tenant 격리 | 기준 환경 | M |
| 3 | 지식 생명주기·파일 안전성 | 인증 경계 | M |
| 4 | 비동기 인덱싱·복원력 | 지식 흐름 안정 | L |
| 5 | RAG 품질 dataset | 재현 fixture | M |
| 6 | 성능·quota·관측성 | 안정적 기능과 metric | L |
| 7 | `/v1` 계약·배포 게이트 | 이전 Exit Gate | M |
