...
export function requireStandardPort(component, value) {
  ...
  if (!expected) throw new Error(`알 수 없는 포트 구성요소입니다: ${component}`);

  const port = value === undefined || value === null || value === '' ? expected : Number(value);
  if (!Number.isInteger(port) || port !== expected) {
    throw new Error(`${component} 포트는 로컬·운영 공통 ${expected}로 고정되어야 합니다.`);
  }
  return port;
}
