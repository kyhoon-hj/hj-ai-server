...
2. `검증 환경 구성`을 실행합니다. 플랫폼 관리자가 STORE_A/STORE_B를 만들고 지식 운영자가 버전 fixture 3개를 등록·인덱싱·게시합니다.
3. `역할 권한 검증`과 `테넌트 격리 검증`을 실행합니다. 역할 권한은 6/6, 테넌트 격리는 8/8 PASS인지 확인합니다.
4. `credential 수명주기 검증`을 실행해 신규 appkey 만료, grace 기간의 신·구 키 허용, 종료 후 이전 키 거부와 감사 이벤트가 4/4 PASS인지 확인합니다. 이 시험은 STORE_B appkey를 회전합니다.
5. 자동 계약 검증을 실행합니다. 14/14 PASS, SKIP 0인지 확인합니다.
6. Swagger 경로와 CORS 허용 Origin을 대상 환경 설정에 맞춘 뒤 `HTTP 노출 보안 검증` 4/4 PASS를 확인합니다.
7. `운영 API 경계 검증`을 실행해 legacy·개발 endpoint 차단과 관리자 대체 경로가 8/8 PASS인지 확인합니다.
8. 전체 기능에서 추가 endpoint와 파일 업로드·다운로드를 수동 확인합니다.
9. 성능 검증을 동시 1부터 시작합니다.
10. 작업이 끝나면 `데모 데이터 정리 확인`을 선택하고 `검증 데이터 정리`를 실행합니다.
